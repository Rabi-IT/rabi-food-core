package gateway

import (
	"bytes"
	"context"
	"encoding/json"
	"fmt"
	"net/http"

	"github.com/Rabi-IT/rabi-food-core/libs/errs"
)

type signInRequest struct {
	Email    string `json:"email"`
	Password string `json:"password"`
}

func (g *GoTrueGatewayAdapter) SignIn(ctx context.Context, input SignInInput) (*TokenOutput, error) {
	body := signInRequest(input)

	raw, err := json.Marshal(body)
	if err != nil {
		return nil, fmt.Errorf("%w: %w", errs.ErrAuthServiceFailure, err)
	}

	req, err := http.NewRequestWithContext(ctx, http.MethodPost, g.baseURL+"/token?grant_type=password", bytes.NewReader(raw))
	if err != nil {
		return nil, fmt.Errorf("%w: %w", errs.ErrAuthServiceFailure, err)
	}

	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("Authorization", "Bearer "+g.serviceKey)

	resp, err := g.httpClient.Do(req)
	if err != nil {
		return nil, fmt.Errorf("%w: %w", errs.ErrAuthServiceFailure, err)
	}
	defer resp.Body.Close()

	if resp.StatusCode == http.StatusBadRequest || resp.StatusCode == http.StatusUnauthorized {
		return nil, wrapResponseError(errs.ErrInvalidCredentials, resp)
	}

	if resp.StatusCode != http.StatusOK {
		return nil, wrapResponseError(errs.ErrAuthServiceFailure, resp)
	}

	var result TokenOutput
	if err := json.NewDecoder(resp.Body).Decode(&result); err != nil {
		return nil, fmt.Errorf("%w: %w", errs.ErrAuthServiceFailure, err)
	}

	return &result, nil
}
